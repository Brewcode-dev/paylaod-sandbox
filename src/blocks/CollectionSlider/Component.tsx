...
export const CollectionSliderBlock: React.FC<Props> = ({
  ...
  return (
    <div className={cn('mx-auto my-8 w-full', themeClasses[styling?.theme || 'light'], className)}>
      {title && (
      ...
      <div className="relative py-6">
        <Swiper ref={swiperRef} {...swiperConfig} className="w-full">
          {items.map((item) => {
            ...
            const date =
              item[config.dateField] || item.createdAt || item.updatedAt || item.lastSynced
            const image =
            ...
            const content =
              item[config.contentField] || item.content || item.description || item.excerpt || title
            return (
              <SwiperSlide key={item.id}>
                <div className={cn('h-full p-4', cardStyleClasses[styling?.cardStyle || 'modern'])}>
                  {/* Item Image */}
        ...
        {slider?.nav && (
          <>
            <button
              className={`${sliderId.current}-prev swiper-button-prev absolute left-4 top-1/2 transform -translate-y-1/2 z-20 bg-white bg-opacity-80 hover:bg-opacity-100 rounded-full p-3 transition-all duration-300 shadow-lg`}
            >
            ...
            <button
              className={`${sliderId.current}-next swiper-button-next absolute right-4 top-1/2 transform -translate-y-1/2 z-20 bg-white bg-opacity-80 hover:bg-opacity-100 rounded-full p-3 transition-all duration-300 shadow-lg`}
            >
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path
                  strokeLinecap="round"
        ...
        {slider?.pagination && (
          <div
            className={`${sliderId.current}-pagination swiper-pagination absolute bottom-4 left-1/2 transform -translate-x-1/2 z-20`}
          />
        )}
      </div>
    </div>
